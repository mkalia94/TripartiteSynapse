...
largeECS = ['phys_right.mat', 'phys_left.mat', 'path_left.mat', 'path_right.mat', 'phys_branch_right.mat', 'phys_branch_right_up.mat']
...
for name_ in smallECS:
    ...
    for i in range(shape(xtemp)[1]):
        ...
        # x_ = append(x_,[0;0;0;0])
        p_ = xtemp[-1,i]*100
        # Voli = fm.model(0,x_,'Voli')
        Voli = xtemp[-3,i]/2*100
        ctr_new = 0
        if i!=0:
            eig_ = eigs[:,i]
...
# plt.savefig('BifFiles/Plots.pdf', format='pdf',bbox_inches='tight',pad_inches=0)
