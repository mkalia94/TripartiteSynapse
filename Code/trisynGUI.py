...
def updateModel():
        ...
        paramdict.pop("excite",None)
        paramdict.pop("astblock",None)
                
        if tabId[0:-3] == 'Excitation':
                paramdict["excite"] = [paramdict["StartExcitation"], paramdict["EndExcitation"], paramdict["Current"], paramdict["Wavelength"], paramdict["Duty"]]
                paramdict["alphae0"] = paramdict["ECS"]/100
                if paramdict['BlockAstrocyte']:
                        ast_ = '\n Astrocyte is blocked during the stimulation. '
                        paramdict["astblock"] = [paramdict['StartExcitation'], paramdict['EndExcitation']]
                else:
                        ast_ = ''
                if paramdict["nogates"]:
                        nogates_ = '\n Steady state gates are used to speed up simulation. '
                else:
                        nogates_ = ''        
                
                paramdict["experiment"] = 'Intial ECS volume fraction is '+ "{:.2f}".format(paramdict["ECS"])+ '%. \n Neuron is stimulated with '+str(paramdict["Current"])+ ' mV current between t='+ str(paramdict["StartExcitation"]) + 'min. and t='+str(paramdict["EndExcitation"]) + 'min. \n The stimulation lasts for '+"{:.2f}".format(paramdict["Wavelength"]) + ' s, every '+ "{:.2f}".format(paramdict["Wavelength"]/60/(1-paramdict["Duty"])) + ' min. ' + ast_ + nogates_

        elif tabId[0:-3] == 'EnergyDeprivation':
                paramdict["perc"] = paramdict["EnergyAvailable"]/100
                paramdict["alphae0"] = paramdict["ECS"]/100
                if paramdict["nogates"]:
                        nogates_ = '\n Steady state gates are used to speed up simulation. '
                else:
                ...
                if paramdict['perc'] > 0.0001:
                        perc_ = 'restricted to ' + "{:.2f}".format(paramdict["EnergyAvailable"]) + '% of baseline capacity, \n'
                else:
                        perc_ =  'completely blocked '


                paramdict['experiment'] = 'Intial ECS volume fraction is '+ "{:.2f}".format(paramdict["ECS"])+ '%. \n Neuron and astrocyte NKA is '+ perc_ +'between t= '+"{:.2f}".format(paramdict["tstart"]) + 's and t= '+"{:.2f}".format(paramdict["tend"]) + 's. '+nogates_      
        
        global currentModel
        paramdict['status'] = 'Computing parameters based on user input...'
        updateModelInfo(paramdict)
        
        currentModel = tps.fmclass(paramdict) # create a model (__init__.py of tps)
        updateModelInfo(currentModel.__dict__)
        
        tps.negcheck_init(currentModel) # execute function negcheck (tps/exec)
        updateModelInfo(currentModel.__dict__)
...
def solveModel():
        ...
        fm.status = 'Solving done...'
        updateModelInfo(fm.__dict__)
        lastSolvedModel = fm

        # resampling if needed
        dt = 1/(60*10)# a reasonable dt (0.1 s)
        if np.min(np.diff(timeArr)) < dt:
                currentModel.status = 'Resampling...'
                updateModelInfo(currentModel.__dict__)
                tStart = timeArr[0]
                tFinal = timeArr[-1]
...
def solvedModelCallback(varIn=None):
        currentModel.status = 'Simulation done! Ready to plot.'
        updateModelInfo(currentModel.__dict__)
        #stopWaitingWidget()
        showGraphOptions()
        initPlotWidow()
...
def solveBtnFunction():
        ...
        currentModel.status = 'Simulation started...'
        updateModelInfo(currentModel.__dict__)
...
def updateModelInfo(dict_):
        modelInfo = mf.getModelInfo()
        for modelInfoIdx,modelInfoVal in enumerate(modelInfo):
                if isinstance(dict_[modelInfoVal[0]],bool):
                        if dict_[modelInfoVal[0]]:
                                newLabel = "True"
                        else:
                                newLabel = "False"
                elif isinstance(dict_[modelInfoVal[0]],float):
                        newLabel = str(round(dict_[modelInfoVal[0]],4))
                else:
                        newLabel = str(dict_[modelInfoVal[0]])
...
def plotModel():
        ...
        if not firstPlot:
                ...
                modelOutputExp = mf.getModelOutputExp()
                for itemIdx, itemLabel in enumerate(graphsToPlot):
                        if graphsToPlot[itemLabel]:
                                plot_exp = modelOutputExp[itemLabel]["exp"]
                                plot_label = modelOutputExp[itemLabel]["pl"] 
                                dataLine = currentModel.model(np.array(timeArr),outputData,plot_exp)
                                axes.plot(timeArr, dataLine, label=plot_label)
...
def plotSingleSubplot(leftAxis, subplotIdx, highlightCurrent=False):
        # plot lines
        rightAxis = None
        modelOutputNames = mf.getModelOutputExp()
        for lineIdx,lineObj in enumerate(graphData.subplots[subplotIdx].lines):
                print(lineObj.name)
                dataLine = lastSolvedModel.model(np.array(graphData.timeData),graphData.dataset,modelOutputNames[lineObj.name]["exp"])
                if lineObj.isLeftAxis:
                        axToUse = leftAxis
                else:
                        if rightAxis == None:
                ...
                axToUse.plot(graphData.timeData, lineObj.offset+lineObj.weight*dataLine, label=modelOutputNames[lineObj.name]["pl"], color=lineObj.color)
...
def addCustomLine():
        ...
        if newLine == None:
                ...
                return
        try:
                lastSolvedModel.model(np.array(graphData.timeData),graphData.dataset,newLine)
        except:
                # line not defined
...
modelInfo = mf.getModelInfo()
app.startLabelFrame("experimentInfo", label="Info", row=2,column=1)
app.setSticky("we")
for modelInfoIdx,modelInfoVal in enumerate(modelInfo):
        app.addLabel(modelInfoVal[0]+"_expInfoLabel",modelInfoVal[1]+":",row=modelInfoIdx,column=0)
        app.setLabelAlign(modelInfoVal[0]+"_expInfoLabel","left")
